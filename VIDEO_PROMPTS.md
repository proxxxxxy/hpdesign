...
支給された3本を内容に合わせて組み込み済みです。上のプロンプト番号と支給ファイル番号は一致しません。

| 元素材 | 掲載場所 | Web用ファイル | 設定項目 |
| --- | --- | --- | --- |
| `gemini動画一覧/1.mp4` | トップの森林住宅 | `assets/hero.mp4` | `heroVideo` |
| `gemini動画一覧/2.mp4` | 工房紹介の本棚・カウンター | `assets/atelier-film.mp4` | `atelierVideo` |
| `gemini動画一覧/3.mp4` | 中盤の光が差す室内 | `assets/interior-film.mp4` | `interiorVideo` |

各映像には同名の `*-poster.webp` を用意しています。差し替える場合はWeb用動画とポスターをセットで更新し、`npm test`・`npm run check`・`npm run build` 後に完成版を反映します。

3本ともミュート・ループ・インライン再生に対応し、画面に入った映像だけ読み込みます。停止ボタン、画面外と非表示タブでの停止、読み込み失敗時の静止画表示を用意しています。動きを減らす設定や通信量節約設定では再生ボタンから開始できます。Geminiの表示は残しています。上記03の手仕事の映像は今後の追加候補です。

原本はそのまま保管し、サイトへ組み込んだWeb用素材だけをコードと一緒にGitHubへ反映します。
